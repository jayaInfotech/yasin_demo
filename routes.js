...
function checkiletype(file,cb)
{
    console.log("myfile "+file.fieldname); 
    console.log("mimetype ",file.mimetype);
    const  filetypes = /gif|jpg|jpeg|png|/;
    const extname = filetypes.test((mime.getExtension(file.mimetype)).toLowerCase());
    console.log(extname);
    // const filesize  = parseInt(file.filesize)/(1028*2);
        ...
        uploads(req,res,function(err){
            ...
            }else
            {
                ...
                for(var i=0;i<req.files.length;i++)
                {
                    ...
                    type : mime.getExtension(req.files[i].mimetype),
                    created : Date.now(),
                    download : 0 ,
                    views : 0
                });
